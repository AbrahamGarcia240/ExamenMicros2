...
extern void comandoLCD(unsigned char comando);
...
{       
    ...
    //inicializamos los UART
    /*
     SEGUN YO, EL MODULO WIFI TIENE UNA VELOCIDAD DE 115200
     */
    U1MODE=0x0420;
    U1STA=0x8000;
    U1BRG=11;
    ...
    iniInterrupciones();
    //configuiramos UART's para trasmision
    U1MODEbits.UARTEN=1;
    U1STAbits.UTXEN=1;
    U2MODEbits.UARTEN=1;
    U2STAbits.UTXEN=1;      
...
void iniInterrupciones( void )
{
    //Habilitacion de interrupcion del sensor HALL
    IFS0bits.INT0IF = 0;
    INTCON2bits.INT0EP = 0;
    IEC0bits.INT0IE = 1;
    //Habilitacion de interrupcion del UART 1
    IFS0bits.U1RXIF = 0;
    IEC0bits.U1RXIE = 1;
...
void iniPerifericos( void )
{
    // estos son los puertos usados para la LCD //
    PORTB=0;
    LATB=0;
    TRISB=0;
    ...
    // el puerto A en el pin 17 es usado para el sensor HALL
    PORTA=0;
    LATA=0;
    TRISA=0;
    PORTAbits.RA11=1;
    ...
    // puertos U1AX usados para el WIFI
    PORTC=0;
    LATC=0;
    TRISCbits.TRISC13=0;
    TRISCbits.TRISC14=1;
